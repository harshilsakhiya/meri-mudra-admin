...
const RegisterPage = () => {
  ...
  const validateForm = () => {
    ...
    if (Loginvalue && !Loginvalue.username) {
      formIsValid = false
  ...
  const signUp = () => {
    if (validateForm()) {
      axios
        .post('http://localhost:5000/api/auth/register', { ...Loginvalue, role: 'Staff' })
        .then(res => {
          console.log('res', res)
          if (res.data.success === true) {
            ...
            toast.error(res.data.error.errors.message)
          }
        })
        .catch(err => {
      ...
      <Card sx={{ zIndex: 1 }}>
        <CardContent sx={{ padding: theme => `${theme.spacing(12, 9, 7)} !important` }}>
          ...
          <div noValidate autoComplete='off'>
            ...
            />
            <Button fullWidth size='large' onClick={() => signUp()} variant='contained' sx={{ marginBottom: 7 }}>
              Sign up
            </Button>
